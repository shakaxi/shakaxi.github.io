...
- =Pair= :: 定义在头文件 =<utility>= 中的 =pair= 模板, 用于存储两种不同类型的元素. 这称为存储异构元素. 每个值的类型都应该在编译时确定. C++14添加了一个功能, 允许根据类型使用 =std::get<T>()= 从tuple中提取元素, 其中T是要提取的元素的类型(而不是索引). 创建tuple的另一种方法是使用 =std::make_tuple()= 工具函数. 这个函数只需要实际值, 在编译时会自动推导出类型(所以不能通过 =&= 来指定引用). 如果需要通过 =make_tuple()= 生成一个包含引用或常量引用的tuple, 那么需要分别使用定义在头文件 =<functional>= 中的 =ref()= 和 =cref()=. 通过 =std::tuple_cat()= 可将两个tuple串联为一个tuple.
